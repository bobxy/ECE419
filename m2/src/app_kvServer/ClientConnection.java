...
public class ClientConnection implements Runnable {
				...
				try {
					...
					if (latestMsg.getMsg().equals("")) {
						...
						if (sValue == null || sValue.equals("null"))
							sValue = "";
						String sRet = "";
						if (message.getStatus() == StatusType.PUT_WITHOUT_CACHING) {
							kvs.putNoCache(sKey, sValue);
							sRet = util
									.StatusCodeToString(StatusType.PUT_SUCCESS)
									+ " " + sKey + " " + sValue;
							sendMessage(new TextMessage(sRet));
						}
						else if (kvs.IsAcceptingRequest()) {
							if (message.getStatus() == StatusType.PUT) {
								if (!kvs.IsLocked()) {
									if (kvs.IsResponsible(sKeyHash,
										...
										String[] correctServer = kvs
												.getResponsible(sKey).trim()
												.split("\\s+");
										if (correctServer.length == 2)
											sRet = util
													.StatusCodeToString(StatusType.SERVER_NOT_RESPONSIBLE)
													+ " "
													+ correctServer[0]
													+ " " + correctServer[1];
										else
											sRet = util
													.StatusCodeToString(StatusType.PUT_ERROR);
									}
								} else {
									sRet = util
											.StatusCodeToString(StatusType.SERVER_WRITE_LOCK)
											+ " " + sKey + " " + sValue;
								}
								...
								if (kvs.IsResponsible(sKeyHash,
									...
									if (correctServer.length == 2)
										...
										sRet = util
												.StatusCodeToString(StatusType.GET_ERROR);
								}
							}
							sendMessage(new TextMessage(sRet));
							/*
							 * connection either terminated by the client or
							 * lost due to network problems
							 */

						} else {
							sRet = util
									.StatusCodeToString(StatusType.SERVER_STOPPED);
							sendMessage(new TextMessage(sRet));
						}
					}
	...
	private TextMessage receiveMessage() throws IOException {
		...
		while (/* read != 13 && */read != 10 && read != -1 && reading) {/*
																		 * CR,
																		 * LF,
																		 * error
																		 */
